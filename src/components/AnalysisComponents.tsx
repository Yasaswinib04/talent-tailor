    ...
    <div className="flex items-center justify-between">
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-2">
          <Badge className="bg-slate-100 text-slate-600 border-none text-[8px] font-black tracking-widest uppercase px-2 py-0.5 rounded-md">{roleType}</Badge>
          <Badge className="bg-primary/10 text-primary border-none text-[8px] font-black tracking-widest uppercase px-2 py-0.5 rounded-md">{experienceTier}</Badge>
        </div>
      </div>
      <div className="flex flex-col items-end">
        <div className="text-2xl font-black text-primary tracking-tighter">{(atsScore * 10).toFixed(0)}%</div>
        <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest opacity-60">Match Index</div>
      </div>
    </div>

    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="p-5 bg-emerald-50/50 rounded-md border border-emerald-100 space-y-3">
      ...
      <div className="p-5 bg-rose-50/50 rounded-md border border-rose-100 space-y-3">
            ...
            return (
              <Badge
                ...
                className={cn(
                  ...
                  isConfirmed
                    ? "bg-primary/10 border-indigo-200 text-primary shadow-sm"
...
export const ImprovementStep = ({ active, number, title, onClick }: { active: boolean; number: number; title: string; onClick: () => void }) => (
  <button
    onClick={onClick}
    className={cn(
      "flex-1 flex flex-col items-center gap-3 p-4 rounded-md transition-all border",
      active ? "bg-primary border-primary text-white shadow-xl shadow-primary/20" : "bg-white border-slate-100 text-slate-400 hover:border-primary/20 hover:text-slate-600"
    )}
  >
    <div className={cn(
      "h-8 w-8 rounded-md flex items-center justify-center font-mono text-xs font-black",
...
export const CandidateCard = ({
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-md border border-slate-200 p-6 cursor-pointer transition-all flex flex-col gap-5 group"
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-md bg-slate-900 text-white flex items-center justify-center font-mono text-[10px] font-black">
            #{rank.toString().padStart(2, '0')}
          </div>
          {c.meetsMandatoryCriteria === false ? (
            <Badge className="bg-rose-50 text-rose-600 border-rose-100 rounded-md text-[8px] font-black uppercase px-2 shadow-none gap-1.5">
              <ShieldAlert className="h-2.5 w-2.5" /> High Risk
            </Badge>
          ) : (
            <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 rounded-md text-[8px] font-black uppercase px-2 shadow-none gap-1.5">
      ...
      <div className="flex justify-between items-start">
        <div className="space-y-1">
          <h3 className="text-lg font-black text-slate-900 leading-none group-hover:text-primary transition-colors uppercase tracking-tight">{c.name}</h3>
          <div className="flex items-center gap-2">
      ...
      <div className="space-y-3">
        <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex justify-between">
          ...
          <span className="text-primary">Top Skills</span>
        </div>
...
export const CandidateLeaderboard = ({ candidates, onDeepDive }: {
  ...
  <div className="space-y-8">
    <div className="flex items-center justify-between">
      ...
      <Badge variant="outline" className="font-mono text-[9px] border-slate-200 text-slate-500 rounded-md px-3 py-1 bg-white shadow-sm">
...
export const CompetencyCard: React.FC<{
  ...
  return (
    <motion.div
      ...
      className={cn(
        "p-4 rounded-md border transition-all flex flex-col gap-3",
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={cn(
            "h-8 w-8 rounded-md flex items-center justify-center font-black text-sm",
      ...
      <div className="mt-2 space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="h-8 px-3 bg-primary text-white rounded-md text-[10px] font-bold hover:bg-primary/90"
          >
            REFINE
          </button>
        </div>
...
export const FeedbackItem: React.FC<FeedbackItemProps> = ({
  ...
  return (
    <div className={cn(
      "p-3 rounded-md border transition-all space-y-3",
      ...
      <div className="space-y-2 pt-1 border-t border-black/5">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="h-7 px-2 bg-primary text-white rounded-md text-[9px] font-bold hover:bg-primary/90"
          >
            REFINE
          </button>
        </div>
...
export const Editor = ({ original, tailored, onTailor, isTailoring, onUpdate }: {
  ...
  if (!tailored && !isTailoring) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-slate-50/50 rounded-md border-2 border-dashed border-slate-200 p-12 text-center space-y-6">
        ...
        <button
          onClick={onTailor}
          className="px-8 py-3 bg-primary text-white font-bold rounded-md hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 flex items-center gap-2"
        >
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex bg-slate-100 p-1 rounded-md">
          <button
            ...
            className={cn(
              ...
              activeView === 'review' ? "bg-white text-primary shadow-sm" : "text-slate-400"
            )}
          >
            Review Changes
          </button>
          <button
            ...
            className={cn(
              "px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all",
              activeView === 'preview' ? "bg-white text-primary shadow-sm" : "text-slate-400"
            )}
          >
            Final Preview
          </button>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 text-slate-400 hover:text-primary hover:bg-primary/10 rounded-md transition-all"
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-md text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all"
          >
      ...
      <div className="flex-1 overflow-hidden">
        {isTailoring ? (
          <div className="flex h-full items-center justify-center bg-white border border-slate-200 rounded-md shadow-sm">
            <div className="flex flex-col items-center gap-4">
              <div className="relative">
                <div className="h-16 w-16 bg-primary/10 rounded-full animate-pulse" />
                <RefreshCw className="h-8 w-8 text-primary animate-spin absolute inset-0 m-auto" />
              </div>
          ...
          <div className="grid grid-cols-2 gap-8 h-full">
            <div className="space-y-4 h-full flex flex-col">
              ...
              <ScrollArea className="flex-1 bg-slate-50 p-6 rounded-md border border-slate-200">
                ...
                <div className="space-y-4">
                  {tailored?.changes.map((change) => (
                    <div
                      key={change.id}
                      className={cn(
                        "p-5 rounded-md border transition-all duration-300",
                        change.accepted
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <Badge className={cn(
                          ...
                          change.accepted ? "bg-primary text-white" : "bg-slate-200 text-slate-400"
                        )}>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="pt-2 bg-slate-50 p-3 rounded-md flex items-start gap-2">
          ...
          <div className="h-full bg-white border border-slate-200 rounded-md shadow-sm p-12 overflow-hidden flex flex-col">
            <div className="flex items-center gap-4 mb-8 border-b border-slate-100 pb-8 shrink-0">
              <div className="h-12 w-12 bg-slate-900 rounded-md flex items-center justify-center text-white font-black">AI</div>
              <div>
...
export const ExtensionPlaceholder = () => (
  <div className="bg-linear-to-br from-indigo-600 to-violet-500 text-white p-5 rounded-md relative overflow-hidden">
